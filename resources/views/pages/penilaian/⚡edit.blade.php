...
new class extends Component {
    ...
    public function loadAlternatif(int $id): void
    {
        ...
        // Set form DAN pre-load selections yang sudah ada
        $this->form->setAlternatif($alternatif);

        Flux::modal('edit-penilaian-modal')->show();
    }

    public function save(): void
    {
        $this->form->update();
        Flux::modal('edit-penilaian-modal')->close();
        session()->flash('success', "Penilaian untuk \"{$this->alternatif->nama}\" berhasil diperbarui.");
...
<div>
    <flux:modal name="edit-penilaian-modal" class="md:w-[560px]">
        <form wire:submit.prevent="save" class="space-y-5">

            {{-- Header --}}
            <div class="pb-2 border-b border-zinc-100 dark:border-zinc-800">
                <flux:heading size="lg">{{ __('Edit Penilaian') }}</flux:heading>
                @if ($alternatif)
                    <flux:text class="mt-1">
                        Ubah sub kriteria untuk
